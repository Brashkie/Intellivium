...
> **v0.10.0 · on npm.** The engine is tested and trains real models. It's still pre-1.0, so the API may evolve — and the grand vision further down is a roadmap, not a current claim.

**Available today** ✅
- Reverse-mode automatic differentiation (Wengert tape, no `Rc<RefCell>`).
- Ops: `matmul`, broadcasted bias `add`, `relu`, `sigmoid`, `tanh`, `MSE`.
- `Dense` layers with He init, sequential `Model`, **SGD & Adam** optimizers, **MSE / BCE / categorical cross-entropy** losses, **softmax** for **multi-class** output, extra activations (**LeakyReLU / ELU / GELU**) and **MAE / Huber** losses, **tensor ops & views** (add/sub/mul, matmul, reductions, reshape/transpose/slice), plus **Dropout**, **LayerNorm**, **BatchNorm** and **Embedding** layers, **mini-batch** training with `Dataset`/`DataLoader`, **validation + early stopping + checkpoints**, custom `Dataset`, weight export/import, **gradient clipping**, **LR decay**, and model **`save`/`load`**.
...
### Phase 1 — Deep Learning Core · 🟡 (only multi-dtype pending)
